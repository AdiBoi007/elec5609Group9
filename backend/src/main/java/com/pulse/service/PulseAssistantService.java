...
public class PulseAssistantService {
    ...
    public PulseAnswer ask(String email, String question, Long conversationId) {
        ...
        PulseAnswer answer = compose(email, question, history, consent.aiAllowed(user));
    ...
    private PulseAnswer compose(String email, String question, List<AiService.Turn> history, boolean allowAi) {
        ...
        if (has(q, "which goal", "goal needs", "most attention", "哪个目标", "最需要")) {
            ...
            if (!ranked.isEmpty()) {
                ...
                List<String> evidence = ranked.stream().limit(3)
                    ...
                    .toList();
                return answer(allowAi, question, history, context, "Goal priority", priority.title() + " currently needs the most attention. Focus on the smallest repeatable action that moves this metric, then reassess after another week of consistent logging.", evidence,
        ...
        if (has(q, "goal", "目标") && !active.isEmpty()) {
            ...
            return answer(allowAi, question, history, context, "Your goals", "You have " + active.size() + " active goal" + (active.size() == 1 ? "" : "s") + ". " + priority.title() + " has the most room to move, so making its next action easy and specific is likely to create the best momentum.", evidence,
        ...
        if (has(q, "water", "hydrat", "喝水", "水分", "补水", "饮水")) {
            ...
            return answer(allowAi, question, history, context, "Hydration today", remaining == 0 ? "You have reached today’s hydration target. Continue drinking according to thirst and the demands of your activity and environment." : "You are at " + percentage + "% of today’s hydration target, with " + remaining + " ml remaining. Use comfortable increments across the rest of the day rather than trying to drink it all at once.",
        ...
        if (has(q, "week", "analyse", "analyze", "overall", "progress", "matter most",
            ...
            return answer(allowAi, question, history, context, "Your week in focus", "Your strongest recorded signal this week is " + strongest + ", while " + weakest + " has the clearest room to improve. Keep the strong behavior stable and choose one small action for the weaker area rather than trying to optimise everything at once.",
    ...
    private PulseAnswer answer(boolean allowAi, String question, List<AiService.Turn> history, Map<String, Object> context, String title, String summary, List<String> evidence, List<PulseAction> actions) {
        AiService.ChatText enhanced = ai.chat(question, history, context, new AiService.ChatText(title, summary, evidence, false), allowAi);
