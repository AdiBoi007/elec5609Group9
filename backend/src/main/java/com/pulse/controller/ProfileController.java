...
public class ProfileController {
    ...
    @GetMapping Map<String, Object> get(Authentication auth) {
        ...
        Map<String, Object> result = new LinkedHashMap<>(); result.put("name", user.getName()); result.put("email", user.getEmail()); result.put("age", p.getAge() == null ? 0 : p.getAge()); result.put("gender", p.getGender() == null ? "" : p.getGender()); result.put("height", p.getHeightCm() == null ? 0 : p.getHeightCm()); result.put("weight", p.getWeightKg() == null ? 0 : p.getWeightKg()); result.put("activityLevel", p.getActivityLevel() == null ? "" : p.getActivityLevel()); result.put("fitnessGoal", p.getFitnessGoal() == null ? "" : p.getFitnessGoal()); result.put("dietaryPreferences", p.getDietaryPreferences() == null ? "" : p.getDietaryPreferences()); result.put("dislikedIngredients", p.getDislikedIngredients() == null ? "" : p.getDislikedIngredients()); result.put("dietaryProfile", dietaryProfiles.response(p)); result.put("dietarySummary", dietaryProfiles.summary(p)); result.put("calorieTarget", p.getCalorieTarget()); result.put("proteinTarget", p.getProteinTarget()); result.put("carbTarget", p.getCarbTarget()); result.put("fatTarget", p.getFatTarget()); result.put("hydrationTargetMl", p.getHydrationTargetMl());
    ...
    @PutMapping @Transactional Map<String, String> update(Authentication auth, @Valid @RequestBody ProfileUpdate body) {
        ...
        if (body.gender() != null) p.setGender(body.gender()); if (body.activityLevel() != null) p.setActivityLevel(body.activityLevel()); if (body.fitnessGoal() != null) p.setFitnessGoal(body.fitnessGoal());
    ...
    public record ProfileUpdate(@Size(max = 100) String name,
        ...
        @Pattern(regexp = "(?i)lose weight|maintain|build muscle", message = "must be a supported value") @Size(max = 60) String fitnessGoal,
    ...
    @GetMapping(value = "/export", produces = "text/csv") @Transactional(readOnly = true) void export(Authentication auth, HttpServletResponse response) throws IOException {
        ...
        writer.printf("%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s%n%n", csv(user.getName()), csv(user.getEmail()), p.getAge(), csv(p.getGender()), p.getHeightCm(), p.getWeightKg(), csv(p.getActivityLevel()), csv(p.getFitnessGoal()), p.getCalorieTarget(), p.getProteinTarget(), p.getCarbTarget(), p.getFatTarget(), p.getHydrationTargetMl());
