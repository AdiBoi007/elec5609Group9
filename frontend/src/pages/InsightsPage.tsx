...
import { AiNotice, AiOffBanner } from "../components/AiConsent";
import { useAiConsent, type AiConsentState } from "../hooks/useAiConsent";
...
export default function InsightsPage() {
  ...
  const ask = useCallback(async (value = question) => {
    ...
    if (!(await consent.confirm("ask"))) return;
  ...
  }, [addError, asking, consent, conversationId, id, question]);
  ...
  if (aiTab !== "ask") return <><AiOffBanner consent={consent}/><CirclePlanGenerator mode={aiTab as "meal-plan" | "workout-plan"} onTabChange={changeTab} consent={consent}/>{consent.dialog}</>;

  return (
    <>
    <CircleAiTabs value="ask" onChange={changeTab}/>
    <AiOffBanner consent={consent}/>
    <div className="mx-auto flex min-h-[calc(100vh-112px)] max-w-[960px] flex-col">
        ...
        {messages.length === 0 ? (
          <section className="mx-auto flex min-h-[54vh] max-w-2xl flex-col items-center justify-center text-center">
            ...
            <p className="mt-6 max-w-lg text-[11px] leading-5 text-muted">
              Your conversations are saved to your account so you can pick up where you left off, and you can delete them at any time.{" "}
              {consent.allowed
                ? "Questions and the health data behind them are sent to OpenAI to generate each answer. Your name and email are never included."
                : "While AI features are off, answers come from Circle Health's built-in guidance and nothing is sent to OpenAI."}{" "}
      ...
      <footer className="sticky bottom-0 z-10 bg-gradient-to-t from-canvas via-canvas to-canvas/0 pb-2 pt-6">
        ...
        <AiNotice consent={consent} feature="ask" className="text-center" />
      </footer>
    </div>
    {consent.dialog}
    </>
...
function CirclePlanGenerator({ mode, onTabChange, consent }: { mode: "meal-plan" | "workout-plan"; onTabChange: (value: string) => void; consent: AiConsentState }) {
