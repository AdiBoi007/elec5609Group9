...
export type PulseAnswer = { title: string; summary: string; evidence: string[]; actions: Array<{ label: string; to?: string; waterMl?: number }>; disclaimer: string; generatedByAi: boolean; conversationId: number | null };
export type ConversationSummary = { id: number; title: string; lastMessageAt: string };
export type ConversationMessage = { id: number; role: "USER" | "ASSISTANT"; content: string; answer: PulseAnswer | null; createdAt: string };
export type ConversationDetail = { id: number; title: string; messages: ConversationMessage[] };
